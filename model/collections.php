<?php 
    class collections_DB {        
       ...
       //Phương thứ lấy ra bản ghi có điều kiện  where tham số truyền vào là slug
       function getIdData($tenTable, $truong, $slug_dmmenu){
        $sql = "SELECT id FROM $tenTable WHERE $truong = '$slug_dmmenu' ";
        $this-> db->execute($sql);
        if($this-> db->num_row()==0){
            $data = 0;
        }else{
